...
events_viz <- events_df[cumsum(events_df$length)<plot_length,]
data$base <- c(rep(events_df$base, events_df$length), 'A')
ggplot(data=data[1:plot_length,], aes(x=x,y=signal,colour=base))+
  geom_point(aes(group=1))+
  geom_line(aes(group=1))+
  geom_vline(xintercept = cumsum(events_viz$length), linetype="dotted", size=0.3, alpha=0.5)+
  xlab('Time step')+
  ylab('Normalized raw signal')
